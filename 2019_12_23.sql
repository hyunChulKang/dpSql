...
IS
    deptno2 dept.deptno%TYPE;
    dname2 dept.dname%TYPE;
    loc2 dept.loc%TYPE;
BEGIN
...
UPDATE dept_test SET dname = 'ACCOUNTING', loc = 'NEW YORK'
WHERE deptno =10;
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE(dept_row.dname || ', ' || dept_row.loc);   --콘솔출력
END;
/
...
DECLARE
    ...
    --type선언이 완료 , tpye을 갖고 변수를 생성
    
    --자바.ver
    --Class 생성후 해당 class의 인스턴스를 생성(new)
...
INSERT INTO dept_tset VALUES( 99, 'ddit', 'daejeon');
...
--dept 테이블의 row를 여러건 저장 할 수있는 dept_tab TABLE TYPE 선언하여
...
--명시적 : 개발자가 이름으르 붙인 커서. 개발자가 직접 제어하며
--       선언, OPEN, FETCH, CLOSE 단계가 존재
--CURSON  커서이름 IS --커서 선언
--  QUERY
--OPEN 커서이름; --커서OPEN
